module.exports = (players, sumOfHeight) => {
    let result = [];
    sumOfHeight===0?155:sumOfHeight
    for (let i = 0;i<players.length;i++) {
        const player = players[i]
        const resultAux = findComplement(parseInt(player.h_in), sumOfHeight, [], players.slice(i,players.length)).map(r => `-${player.first_name} ${player.last_name}         ${r.name}`)
